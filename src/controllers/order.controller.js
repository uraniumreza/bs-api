...
const updateStock = async product => {
  await Product.updateOne({ _id: product.productId }, { $inc: { quantity: -product.final_quantity } }, error => {
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(orderId)) {
      ...
      if (req.user.role === "user" && order.state === "Processing" && req.body.state === "Delivered") {
